...
public class FireGrassBlock extends ModBlockSpreadable {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!canSurvive(state, world, pos)) {
            world.setBlockState(pos, BlockRegistry.FIRE_DIRT.getDefaultState());
        } else {
            if (world.getLightLevel(pos.up()) >= 9) {
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    if (world.getBlockState(blockPos).isOf(BlockRegistry.FIRE_DIRT) && canSpread(blockState, world, blockPos)) {
